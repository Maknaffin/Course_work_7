...
class HabitAPITestCase(APITestCase):
    ...
    def test_habit_detail(self):
        response = self.client.get(reverse('habits:habit-get', args=[self.habit.id]))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json(),
                         {
                             'id': self.habit.id,
    ...
    def test_habit_update(self):
        ...
        response = self.client.put(reverse('habits:habit-update', args=[self.habit.id]), data=data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json(),
                         {
                             'id': self.habit.id,
    ...
    def test_all_habits(self):
        response = self.client.get(reverse('habits:habit-all'))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json(),
                         {
                             'count': 1,
